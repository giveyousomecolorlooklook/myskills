#!/usr/bin/env python3
"""Script CLI for image generation or editing with GPT Image models.

This is the default execution path for the user-imagegen skill. It always uses
`gpt-image-2`; transparent deliverables use chroma-key post-processing.
...
def _load_config(path: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        _die(f"Invalid JSON config file: {config_path}: {exc}")
    if not isinstance(raw, dict):
        _die(f"Config file must contain a JSON object: {config_path}")
    return raw


def _resolve_api_options(args: argparse.Namespace) -> Tuple[Optional[str], Optional[str]]:
    config = _load_config(getattr(args, "config", None))
    api_key = (
        _clean_optional_string(getattr(args, "api_key", None))
        or _clean_optional_string(config.get("api_key"))
        or _clean_optional_string(config.get("openai_api_key"))
        or _clean_optional_string(os.getenv("IMAGEGEN_API_KEY"))
        or _clean_optional_string(os.getenv("OPENAI_API_KEY"))
    )
    base_url = (
        _clean_optional_string(getattr(args, "base_url", None))
        or _clean_optional_string(config.get("base_url"))
        or _clean_optional_string(config.get("openai_base_url"))
        or _clean_optional_string(os.getenv("IMAGEGEN_BASE_URL"))
        or _clean_optional_string(os.getenv("OPENAI_BASE_URL"))
    )
    return api_key, base_url


def _ensure_api_key(args: argparse.Namespace) -> None:
    api_key, _ = _resolve_api_options(args)
    if api_key:
        print("API key is set.", file=sys.stderr)
        return
    if args.dry_run:
        _warn("API key is not set; dry-run only.")
        return
    _die(
        "API key is not set. Use --api-key, set IMAGEGEN_API_KEY/OPENAI_API_KEY, "
...
def _validate_model(model: str) -> None:
    if model != GPT_IMAGE_2_MODEL:
        _die("model must be gpt-image-2. This skill does not route to another image model.")
...
def _validate_model_specific_options(
    *,
    ...
    if background == "transparent":
        _die(
            "background=transparent is not supported in gpt-image-2. Generate on a flat chroma-key "
            "background with gpt-image-2, then run remove_chroma_key.py for local alpha extraction."
        )
...
def _client_kwargs(args: argparse.Namespace) -> Dict[str, str]:
    api_key, base_url = _resolve_api_options(args)
    kwargs: Dict[str, str] = {}
    if api_key:
...
def _add_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=f"JSON config file with api_key/base_url (default: {DEFAULT_CONFIG_PATH})",
    )
    parser.add_argument("--api-key", help="API key for an OpenAI-compatible provider")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Script CLI for image generation or editing via gpt-image-2"
    )
