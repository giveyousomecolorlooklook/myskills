...
These prompt recipes are for the skill's default `scripts/image_gen.py` workflow.
...
Execution details such as CLI flags, `quality`, masks, output formats, and local output paths belong in the script command, not the image prompt. For transparent images, always use `gpt-image-2`: prompt for a flat chroma-key background and remove it locally with `python "${CODEX_HOME:-$HOME/.codex}/skills/user-imagegen/scripts/remove_chroma_key.py"`.

CLI model notes:
- `gpt-image-2` is the CLI default for new workflows.
- `gpt-image-2` supports `quality` values `low`, `medium`, `high`, and `auto`.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- Always use `gpt-image-2` for transparent output, with chroma-key generation and local alpha extraction.
...
Post-process note: after chroma-key generation, run `python "${CODEX_HOME:-$HOME/.codex}/skills/user-imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. Do not switch models for true/native transparency, failed chroma-key validation, or complex subjects; retry the `gpt-image-2` chroma-key path and report edge-cleanup risk if needed.
