...
These prompting principles are shared by the skill's script workflow.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, output format, and output paths live in the CLI/API docs.
...
- Use CLI `medium` or `high` quality for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Always use `gpt-image-2` for transparent-image requests. Use chroma-key post-processing because direct transparent-background API output is not part of this skill path.
- For chroma-key output, prompt for a perfectly flat solid chroma-key background, usually `#00ff00`; use `#ff00ff` when the subject is green, and avoid key colors that appear in the subject.
...
- After chroma-key generation, remove the background locally with `python "${CODEX_HOME:-$HOME/.codex}/skills/user-imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
- Do not route true/native transparency requests, failed chroma-key validation, or complex transparent subjects to another model. Keep `gpt-image-2`, retry the chroma-key prompt/removal when useful, and report edge-cleanup risk if needed.

## Execution controls
- `quality`, `input_fidelity`, explicit masks, output format, and output paths are script execution controls.
...
- If a transparent request needs true/native transparency, still use `gpt-image-2` with chroma-key generation and local alpha extraction.
...
- background-extraction: Use `gpt-image-2` chroma-key output. Request a clean cutout on a perfectly flat background; crisp silhouette; generous padding; no shadows; no halos; preserve label text exactly; no restyling.
